package net.itsjustbits.vanilla_expanded.entity;
...
import static net.itsjustbits.vanilla_expanded.initializers.VanillaExpMain.nameSpace;

@Environment(EnvType.CLIENT)
public class VanExpArmorRenderer <T extends LivingEntity, M extends BipedEntityModel<T>, A extends BipedEntityModel<T>> extends FeatureRenderer<T, M> {
    private static final Map<String, Identifier> ARMOR_TEXTURE_CACHE = Maps.newHashMap();
    private final A leggingsModel;
    private final A bodyModel;

    public VanExpArmorRenderer(FeatureRendererContext<T, M> context, A leggingsModel, A bodyModel) {
        super(context);
    ...
    private Identifier getArmorTexture(ArmorItem armorItem, boolean bl, @Nullable String string) {
        Identifier id = new Identifier(nameSpace, "textures/models/armor/" + armorItem.getMaterial().getName() + "_layer_" + (bl ? 2 : 1) + (string == null ? "" : "_" + string) + ".png");
        return id;
    }
}
